            ...
            if (data.type === 'run') {
                ...
                switch (language) {
                    case 'python':
                        ...
                        dockerCommand = `echo "${encodedCode}" | base64 -d > /tmp/code.py`;
                        if (encodedInput) {
                            dockerCommand += ` && echo "${encodedInput}" | base64 -d | python3 /tmp/code.py`;
                        }
                        else {
                            dockerCommand += ` && python3 /tmp/code.py`;
                        }
                        break;
                    case 'javascript':
                        image = 'node:18-alpine';
                        dockerCommand = `echo "${encodedCode}" | base64 -d > /tmp/code.js`;
                        if (encodedInput) {
                    ...
                    case 'c':
                        image = 'gcc:latest';
                        dockerCommand = `echo "${encodedCode}" | base64 -d > /tmp/code.c && gcc /tmp/code.c -o /tmp/code`;
                        if (encodedInput) {
                    ...
                    case 'cpp':
                        image = 'gcc:latest';
                        dockerCommand = `echo "${encodedCode}" | base64 -d > /tmp/code.cpp && g++ /tmp/code.cpp -o /tmp/code`;
                        if (encodedInput) {
                            dockerCommand += ` && echo "${encodedInput}" | base64 -d | stdbuf -o0 /tmp/code`;
                        }
                        else {
                    ...
                    case 'java':
                        ...
                        dockerCommand = `echo "${encodedCode}" | base64 -d > /tmp/Main.java && javac /tmp/Main.java`;
                        if (encodedInput) {
